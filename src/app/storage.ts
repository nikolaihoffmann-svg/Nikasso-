...
function normalizeItem(
  input: Partial<InventoryItem> & Record<string, unknown>
): InventoryItem {
  return {
...
function normalizePurchaseCostMode(value: unknown): "last_price" | "no_change" {
  const v = String(value ?? "").trim().toLowerCase();
  if (v === "no_change") return "no_change";
  return "last_price";
}
...
export function getPurchases(): PurchaseRecord[] {
  return readJson<Partial<PurchaseRecord>[]>(PURCHASES_KEY, [])
...
export function updateCustomer(
  ...
  const updated: Customer = {
    ...
    phone:
      patch.phone === ""
        ? undefined
        : (patch.phone ?? current.phone)?.trim() || undefined,
    address:
      patch.address === ""
        ? undefined
        : (patch.address ?? current.address)?.trim() || undefined,
    note:
      patch.note === ""
        ? undefined
        : (patch.note ?? current.note)?.trim() || undefined,
    updatedAt: nowIso(),
  };
...
export function savePurchase(
  ...
  for (const line of draft.lines) {
    ...
    items[index] = {
      ...
      costPrice: draft.updateCostMode === "no_change" ? current.costPrice : unitCost,
      updatedAt: nowIso(),
    };
  }
...
export function importBackupObject(input: unknown): void {
  ...
  const mappedPurchases: PurchaseRecord[] = Array.isArray(data.purchases)
    ? data.purchases.map((purchase) => normalizePurchaseRecord(purchase))
    : [];
