...
function normalizeKind(k: PurchaseKind | undefined): PurchaseKind {
...
function LineChart(props: { title: string; series: { name: string; values: number[] }[]; labels: string[]; sub?: string }) {
  const W = 640;
  const H = 220;
  const pad = 26;
  ...
  return (
    <div className="card">
      ...
      <div className="cardSub">{props.sub ?? `Siste ${props.labels.length} måneder`}</div>
      ...
      <div style={{ overflowX: "auto" }}>
        <svg width={W} height={H} viewBox={`0 0 ${W} ${H}`} role="img" aria-label={props.title} style={{ display: "block" }}>
          ...
          {props.series.map((s, si) => {
            ...
            const stroke =
              si === 0 ? "rgba(79,131,255,0.95)" : si === 1 ? "rgba(255,90,95,0.85)" : "rgba(53,208,127,0.9)";
            return (
              <g key={s.name}>
...
function BarChart(props: { title: string; values: { label: string; value: number }[]; valueFmt?: (n: number) => string }) {
  const max = Math.max(1, ...props.values.map((x) => (Number.isFinite(x.value) ? x.value : 0)));
  const fmt = props.valueFmt ?? ((n) => fmtKr(n));
            ...
            <div key={x.label} className="item" style={{ padding: 10 }}>
              <div className="itemTop" style={{ alignItems: "center" }}>
                <div style={{ minWidth: 0 }}>
                  ...
                  <div className="itemMeta">{fmt(x.value)}</div>
                </div>
                <div style={{ width: 140 }}>
                  <div
                    style={{
...
export function Oversikt() {
  ...
  const purchaseTotals = useMemo(() => getPurchaseTotalsByKind(purchases), [purchases]);
  ...
  const totals = useMemo(() => {
    ...
    // "du skylder" = utestående leverandørgjeld (payables) ligger i Innkjøp-siden,
    // men her viser vi i hvert fall "penger brukt totalt" og kan trekke "du skylder" separat hvis du vil senere.
    const spentTotal = purchaseTotals.total;

    const netWhenSettled = round2(Number(saldo) + unpaidSalesTotal + unpaidReceivablesTotal);
  ...
  }, [sales, receivables, saldo, byItemId, purchaseTotals.total]);
  ...
  const purchaseBreakdown = useMemo(() => {
    // totals: { total, varer, forbruk, utstyr }
    return [
      { label: "Varekjøp", value: purchaseTotals.varer },
      { label: "Forbruk", value: purchaseTotals.forbruk },
      { label: "Utstyr", value: purchaseTotals.utstyr },
    ];
  }, [purchaseTotals]);
  ...
  return (
    <div className="card">
      ...
      {/* Kjøp fordelt */}
      <BarChart title="Innkjøp fordelt" values={purchaseBreakdown} valueFmt={(n) => fmtKr(n)} />
      ...
      {/* Topplister */}
      <BarChart title="Topp kunder (totalt)" values={topCustomersAllTime.map((c) => ({ label: c.name, value: round2(c.revenue) }))} />
      <BarChart title="Topp varer (totalt)" values={topItemsAllTime.map((x) => ({ label: x.name, value: round2(x.revenue) }))} />
